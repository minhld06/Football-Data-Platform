...
4. **DB-role enforcement** (`infra/postgres/migrations/007_chatbot_readonly_role.sql`) — the validated SQL runs under `chatbot_ro`, which only has `SELECT` on schema `gold`. Even if layers 1–3 all failed at once, the role itself can't write, drop, or read outside `gold`. `chatbot_ro` also has a 5s `statement_timeout` (`infra/postgres/migrations/008_chatbot_statement_timeout.sql`), so a query that's technically valid SQL but expensive (e.g. an unintended cross join across the allowed tables, or a slow function call) gets killed instead of tying up a connection indefinitely.
...
- Per-IP rate limiting on `/api/chat` (`backend/rate_limit.py`, 10 requests / 60s) is a plain in-memory sliding window — it resets on backend restart and isn't shared across multiple backend replicas. Fine for this project's single-instance deployment; would need a shared store (e.g. Redis) to scale beyond that.
...
4. **Application au niveau du rôle DB** (`infra/postgres/migrations/007_chatbot_readonly_role.sql`) — le SQL validé s'exécute sous `chatbot_ro`, qui n'a que `SELECT` sur le schéma `gold`. Même si les couches 1 à 3 échouaient toutes en même temps, le rôle lui-même ne peut ni écrire, ni supprimer, ni lire en dehors de `gold`. `chatbot_ro` a aussi un `statement_timeout` de 5s (`infra/postgres/migrations/008_chatbot_statement_timeout.sql`), pour qu'une requête syntaxiquement valide mais coûteuse (ex. jointure croisée non voulue entre les tables autorisées, ou appel de fonction lent) soit interrompue au lieu de bloquer une connexion indéfiniment.
...
- La limitation de débit par IP sur `/api/chat` (`backend/rate_limit.py`, 10 requêtes / 60s) est une simple fenêtre glissante en mémoire — elle se réinitialise au redémarrage du backend et n'est pas partagée entre plusieurs instances. Suffisant pour le déploiement mono-instance de ce projet ; il faudrait un store partagé (ex. Redis) pour aller au-delà.
...
4. **Ép ở mức DB role** (`infra/postgres/migrations/007_chatbot_readonly_role.sql`) — SQL đã validate chạy dưới role `chatbot_ro`, role này chỉ có quyền `SELECT` trên schema `gold`. Kể cả nếu lớp 1–3 cùng lúc bị bypass, bản thân role cũng không thể ghi, xóa, hay đọc ngoài `gold`. `chatbot_ro` còn có `statement_timeout` 5s (`infra/postgres/migrations/008_chatbot_statement_timeout.sql`), để một câu SQL hợp lệ về cú pháp nhưng tốn tài nguyên (vd join chéo ngoài ý muốn giữa các bảng được cho phép, hoặc gọi hàm chạy chậm) bị huỷ thay vì giữ connection vô thời hạn.
...
- Rate limiting theo IP cho `/api/chat` (`backend/rate_limit.py`, 10 request / 60s) chỉ là sliding window lưu trong bộ nhớ — reset khi backend restart và không share được giữa nhiều instance backend. Đủ dùng cho deployment 1 instance của project này; cần store dùng chung (vd Redis) nếu muốn scale ra nhiều instance.
